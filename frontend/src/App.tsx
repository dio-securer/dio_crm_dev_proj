...
export default function App() {
  ...
  const links = [
    ...
    ['/activity-reports', 'nav.activityReport', false], ['/direct-work', 'nav.directWork', false],
    ...
    <main style={{ fontFamily: 'Malgun Gothic, Segoe UI, sans-serif', maxWidth: 1240, margin: '28px auto', padding: 20 }}>
      <header style={{ marginBottom: 20 }}>
        <div style={{ display:'flex', justifyContent:'space-between', gap:16, alignItems:'start', flexWrap:'wrap' }}>
          <div><h1 style={{ marginBottom: 4 }}>{t('app.name')}</h1><p style={{ marginTop: 0, color: '#667085' }}>{t('app.phase')}</p></div>
        ...
        <nav style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {links.map(([to, key, end]) => <NavLink key={to} to={to} end={end} style={linkStyle}>{t(key)}</NavLink>)}
        </nav>
      </header>
      <Routes>
