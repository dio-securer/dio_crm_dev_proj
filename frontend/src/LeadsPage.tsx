...
import {
  ...
  loadMockLeads,
  resetMockLeads,
  updateMockLeadHospitalScale
} from './features/lead/lead-mock-service';
...
export function LeadsPage() {
  ...
  const [openSections, setOpenSections] = useState<Record<SectionId, boolean>>({
    basic: true, nextAction: true, recentActivity: true, tagsNote: false,
    activity: true, keyman: true, hospitalScale: true, system: true, conversion: true
  });
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows
      .filter(row => {
        ...
        return [row.leadNo, row.leadName, row.organizationName, row.phone ?? '', row.ownerName, keyman].some(value => value.toLocaleLowerCase().includes(keyword));
      })
      .filter(row => {
  ...
  const money = (value?: number) => value == null ? '-' : new Intl.NumberFormat(i18n.language, {
    style: 'currency', currency: globalization.currencyCode, maximumFractionDigits: 0
  }).format(value);
  ...
  const updateStage = (stage: LeadStage) => {
    if (!selected) return;
    setRows(changeMockLeadStage(rows, selected.leadId, stage));
    notify(t('lead.toast.statusUpdated'));
  };

  const saveHospitalScale = (value: NonNullable<LeadRecord['hospitalScale']>) => {
    if (!selected) return;
    setRows(updateMockLeadHospitalScale(rows, selected.leadId, value));
    notify(t('lead.toast.hospitalScaleSaved'));
  };

  const addActivity = (input: LeadActivityInput) => {
    if (!selected) return;
    setRows(addMockLeadActivity(rows, selected.leadId, input));
    setPreferredActivityType(null);
    notify(t('lead.toast.activityAdded'));
  };
  ...
  const createLead = (openDetail: boolean) => {
    ...
    const next = createMockLead(rows, {
      leadName: quickName, organizationName: quickOrganization, phone: quickPhone, country: quickCountry,
      address: quickAddress, source: quickSource, ownerUserId: quickOwner
    });
    setRows(next); setSelectedId(next[0].leadId); setDrawerOpen(false); setQuickName(''); setQuickOrganization(''); setQuickPhone('');
    setQuickCountry('KR'); setQuickAddress(''); setQuickSource('WEB'); setQuickOwner('USER001'); setPage(1); setActiveTab('overview');
    setMobileDetailOpen(openDetail); setOpenSections(prev => ({ ...prev, basic: true, nextAction: true, recentActivity: true, keyman: true }));
    notify(t('lead.toast.created'));
  };

  const resetFilters = () => {
    setSearch(''); setListFilter('all'); setStageFilter('ALL'); setOwnerFilter('ALL'); setCountryFilter('ALL'); setRegionFilter('ALL');
    setSourceFilter('ALL'); setLastActivityFilter('ALL'); setPage(1);
  };

  const resetMock = () => {
    const next = resetMockLeads(); setRows(next); setSelectedId(next[0]?.leadId ?? ''); resetFilters(); setActiveTab('overview');
    setMobileDetailOpen(false); notify(t('lead.toast.reset'));
  };

  const goToPage = (nextPage: number) => {
    const clamped = Math.min(Math.max(1, nextPage), totalPages); setPage(clamped);
    const first = filteredRows[(clamped - 1) * pageSize]; if (first) setSelectedId(first.leadId);
  };

  const changePageSize = (nextSize: number) => { setPageSize(nextSize); setPage(1); if (filteredRows[0]) setSelectedId(filteredRows[0].leadId); };
  ...
  const renderDetailTab = (lead: LeadRecord) => {
    if (activeTab === 'overview') return <div className="ab-overview-grid">
      ...
      <AbSectionAccordion id="nextAction" title={t('lead.sections.nextAction')} open={openSections.nextAction} onToggle={() => toggleSection('nextAction')} incomplete={!lead.nextAction}>
        <AbNextAction label={t('lead.fields.nextAction')} title={lead.nextAction ?? '-'} due={formatDate(lead.nextActionAt, i18n.language, true)} action={<button type="button" className="lead-v2-button ghost" onClick={() => openActivityTab()}>{t('lead.tabs.activity')}</button>} />
      </AbSectionAccordion>
      <AbSectionAccordion id="recentActivity" title={t('lead.sections.recentActivity')} hint={lead.activities.length ? `${Math.min(lead.activities.length, 3)}` : undefined} open={openSections.recentActivity} onToggle={() => toggleSection('recentActivity')}>
        <LeadActivitySection activities={lead.activities} onSubmit={addActivity} limit={3} quickAdd={false} />
      </AbSectionAccordion>
    ...
    if (activeTab === 'activity') return <AbSectionAccordion id="activity" title={t('lead.sections.activity')} hint={lead.activities.length ? `${lead.activities.length}` : undefined} open={openSections.activity} onToggle={() => toggleSection('activity')}>
      <LeadActivitySection activities={lead.activities} onSubmit={addActivity} preferredType={preferredActivityType} />
    </AbSectionAccordion>;

    if (activeTab === 'keyman') return <AbSectionAccordion id="keyman" title={t('lead.sections.keyman')} hint={lead.contacts.length ? `${lead.contacts.length}` : undefined} open={openSections.keyman} onToggle={() => toggleSection('keyman')} incomplete={!lead.contacts.length}>
      <div className="lead-v2-contact-list">{lead.contacts.map(contact => <div className="lead-v2-contact lead-v2-contact-wide" key={contact.id}><span>{contact.name.slice(0, 1)}</span><div><strong>{contact.name}</strong><small>{contact.role}</small><em>{[contact.phone, contact.email].filter(Boolean).join(' · ')}</em></div></div>)}{!lead.contacts.length && <p className="lead-v2-empty-inline">{t('lead.empty.keyman')}</p>}</div>
    </AbSectionAccordion>;

    if (activeTab === 'system') {
      const scale = lead.hospitalScale ?? { hospitalType: 'CLINIC', doctorCount: 1, chairCount: 4, staffCount: 6, mainSpecialty: '' };
      return <div className="ab-overview-grid">
        <AbSectionAccordion id="hospitalScale" title={t('lead.sections.hospitalScale')} open={openSections.hospitalScale} onToggle={() => toggleSection('hospitalScale')}>
          <div className="lead-hospital-summary">
            <div><span>{t('lead.fields.doctorCount')}</span><strong>{scale.doctorCount ?? '-'}</strong></div>
            <div><span>{t('lead.fields.chairCount')}</span><strong>{scale.chairCount ?? '-'}</strong></div>
            <div><span>{t('lead.fields.staffCount')}</span><strong>{scale.staffCount ?? '-'}</strong></div>
            <div><span>{t('lead.fields.mainSpecialty')}</span><strong>{scale.mainSpecialty || '-'}</strong></div>
          </div>
          <LeadHospitalScaleForm value={scale} onSave={saveHospitalScale} />
        </AbSectionAccordion>
        <AbSectionAccordion id="system" title={t('lead.sections.system')} open={openSections.system} onToggle={() => toggleSection('system')} incomplete>
          <InfoRow label={t('lead.fields.mainSystem')} value="-" /><InfoRow label={t('lead.fields.subSystem')} value="-" />
        </AbSectionAccordion>
      </div>;
    }
    ...
    return <AbSectionAccordion id="conversion" title={t('lead.sections.conversion')} open={openSections.conversion} onToggle={() => toggleSection('conversion')}>
      ...
      <div className="lead-v2-drawer-actions"><button type="button" className="lead-v2-button primary" onClick={() => notify(t('lead.toast.converted'))}>{t('lead.actions.convert')}</button><button type="button" className="lead-v2-button ghost" onClick={() => updateStage('DISQUALIFIED')} disabled={lead.stage === 'CONVERTED'}>{t('lead.actions.exclude')}</button></div>
    </AbSectionAccordion>;
  };
  ...
  const listPane = <>
    ...
    <AbDataList columns={listColumns} rows={pagedRows} rowKey={lead => lead.leadId} selectedKey={selectedId} onRowClick={lead => selectLead(lead.leadId)} ariaLabel={t('lead.listTitle')} empty={<AbEmptyState title={t('lead.empty.search')} />} renderCells={lead => [
      ...
      <span title={lead.phone}>{lead.phone ?? '-'}</span>, <span>{formatDate(lead.lastActivityAt, i18n.language)}</span>,
  ...
  const detailPane = <>{!selected && <div className="lead-v2-empty-detail">{t('lead.empty.detail')}</div>}{selected && <>
    ...
    <AbDetailHeader eyebrow={selected.leadNo} title={selected.leadName} subtitle={<><strong>{selected.organizationName}</strong> · <span className="ab-list-country"><i>{countryFlag(selected.country)}</i>{selected.country} · {selected.region}</span></>} badges={<AbEntityBadges badges={[{ label: t(`leadV2.stage.${selected.stage}`), tone: selected.stage === 'CONVERTED' ? 'success' : 'info' }, { label: t(`lead.interest.${selected.interestLevel}`), tone: 'neutral' }]} />} meta={<div className="ab-detail-meta-list"><div className="ab-detail-meta-item"><span>{t('lead.fields.owner')}</span><strong>{selected.ownerName}</strong></div><div className="ab-detail-meta-item"><span>{t('lead.fields.lastActivity')}</span><strong>{formatDate(selected.lastActivityAt, i18n.language, true)}</strong></div><div className="ab-detail-meta-item"><span>{t('lead.fields.nextAction')}</span><strong>{selected.nextAction ?? '-'}</strong></div><div className="ab-detail-meta-item"><span>{t('lead.fields.source')}</span><strong>{t(`lead.source.${selected.source}`)}</strong></div></div>} actions={<AbQuickActions ariaLabel={t('lead.quickActionsLabel')} actions={[
    ...
    <AbStepProgress steps={stepLabels} currentIndex={mockStageToStep(selected.stage)} mobileLabel={selected.stage === 'DISQUALIFIED' ? t('lead.status.CONTACT_EXCLUDED') : `${mockStageToStep(selected.stage) + 1}/${API_LEAD_STEPS.length}`} />
    {selected.stage === 'DISQUALIFIED' && <div className="ab-detail-terminal"><strong>{t('lead.status.CONTACT_EXCLUDED')}</strong></div>}
    <div className="ab-detail-control-strip"><label><span>{t('lead.fields.stage')}</span><select value={selected.stage} onChange={event => updateStage(event.target.value as LeadStage)}>{LEAD_STAGES.map(stage => <option key={stage} value={stage}>{t(`leadV2.stage.${stage}`)}</option>)}</select></label><div><span>{t('lead.fields.expectedAmount')}</span><strong>{money(selected.expectedAmount)}</strong></div><div><span>{t('lead.fields.expectedDate')}</span><strong>{formatDate(selected.expectedDate, i18n.language)}</strong></div></div>
    <AbDetailTabs ariaLabel={t('lead.detailTitle')} activeId={activeTab} onChange={selectDetailTab} tabs={[{ id: 'overview', label: t('lead.tabs.overview') }, { id: 'activity', label: t('lead.tabs.activity'), count: selected.activities.length }, { id: 'keyman', label: t('lead.tabs.keyman'), count: selected.contacts.length }, { id: 'system', label: t('lead.tabs.system') }, { id: 'conversion', label: t('lead.tabs.conversion'), count: selected.opportunityCount }]} />
  ...
  return <section className={`lead-v2 ab-workspace${mobileDetailOpen ? ' mobile-detail-open' : ''}`}>
    <header className="lead-v2-page-header"><div><div className="lead-v2-title-line"><span className="lead-v2-kicker">CRM · LEAD</span><span className="lead-v2-mock-badge">{t('lead.mockMode')}</span></div><h2>{t('lead.title')}</h2><p>{t('lead.subtitle')}</p></div><div className="lead-v2-header-actions"><button type="button" className="lead-v2-button ghost" onClick={resetMock}>{t('lead.actions.resetMock')}</button><button type="button" className="lead-v2-button primary" onClick={() => setDrawerOpen(true)}>+ {t('lead.actions.create')}</button></div></header>
    ...
    {drawerOpen && <div className="lead-v2-drawer-backdrop" onMouseDown={() => setDrawerOpen(false)}><aside className="lead-v2-drawer" onMouseDown={event => event.stopPropagation()}><div className="lead-v2-drawer-header"><div><strong>{t('lead.quick.title')}</strong><p>{t('lead.quick.help')}</p></div><button type="button" onClick={() => setDrawerOpen(false)} aria-label={t('app.close')}>×</button></div><div className="lead-v2-form"><label><span>{t('lead.fields.hospitalName')} *</span><input value={quickOrganization} onChange={event => setQuickOrganization(event.target.value)} /></label><label><span>{t('lead.fields.country')} *</span><input value={quickCountry} onChange={event => setQuickCountry(event.target.value)} /></label><label><span>{t('lead.fields.phone')} *</span><input value={quickPhone} onChange={event => setQuickPhone(event.target.value)} inputMode="tel" /></label><label><span>{t('lead.fields.contactName')} *</span><input value={quickName} onChange={event => setQuickName(event.target.value)} /></label><label><span>{t('lead.fields.address')}</span><input value={quickAddress} onChange={event => setQuickAddress(event.target.value)} /></label><label><span>{t('lead.fields.source')}</span><select value={quickSource} onChange={event => setQuickSource(event.target.value as LeadSource)}>{LEAD_SOURCES.map(source => <option key={source} value={source}>{t(`lead.source.${source}`)}</option>)}</select></label><label><span>{t('lead.fields.owner')} *</span><select value={quickOwner} onChange={event => setQuickOwner(event.target.value)}>{MOCK_LEAD_OWNERS.map(owner => <option key={owner.id} value={owner.id}>{owner.name}</option>)}</select></label></div><div className="lead-v2-drawer-actions"><button type="button" className="lead-v2-button primary" onClick={() => createLead(false)}>{t('lead.quick.create')}</button><button type="button" className="lead-v2-button secondary" onClick={() => createLead(true)}>{t('lead.quick.createDetail')}</button><button type="button" className="lead-v2-button ghost" onClick={() => setDrawerOpen(false)}>{t('lead.actions.cancel')}</button></div></aside></div>}
