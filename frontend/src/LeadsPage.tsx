...
export function LeadsPage() {
  ...
  const accountOptions = useMemo(() => listSandboxAccounts('', 'all').map(account => ({ id: account.public_id, name: account.account_name })), [rows]);
  ...
  const updateStage = (stage: LeadStage) => {
    if (!selected || selected.stage === 'CONVERTED') return;
  ...
  const createLeadFromQuick = (value: LeadQuickCreateValue, draft: boolean) => {
    const next = createMockLead(rows, {
      leadName: value.contactName,
      organizationName: value.hospitalName,
      phone: value.phone,
      country: value.country,
      address: value.address,
      source: value.source,
      ownerUserId: value.ownerValue
    });
    const created = next[0];
    setRows(next);
    setSelectedId(created.leadId);
    setPage(1);
    setActiveTab('overview');
    setMobileDetailOpen(true);
    setOpenSections(prev => ({ ...prev, basic: true, nextAction: true, recentActivity: true, keyman: true }));
    notify(draft ? t('lead.toast.draftSaved') : t('lead.toast.created'));
    return true;
  };

  const conversionResultFor = (lead: LeadRecord): LeadConversionResult | null => {
    const stored = getMockLeadConversion(lead.leadId);
    if (stored) return stored;
    if (!lead.convertedAccountId || !lead.convertedAt) return null;
    const account = listSandboxAccounts('', 'all').find(row => row.public_id === lead.convertedAccountId);
    return {
      accountId: lead.convertedAccountId,
      accountName: account?.account_name ?? lead.organizationName,
  ...
  const convertLead = async (lead: LeadRecord, options: LeadConversionOptions) => {
    if (lead.stage === 'CONVERTED' || lead.convertedAccountId) throw new Error('LEAD_ALREADY_CONVERTED');
    ...
    setRows(completeMockLeadConversion(rows, lead.leadId, result));
    notify(t('lead.toast.converted'));
  };
  ...
  const renderDetailTab = (lead: LeadRecord) => {
    if (activeTab === 'overview') return <div className="ab-overview-grid">
      ...
      <AbSectionAccordion id="nextAction" title={t('lead.sections.nextAction')} open={openSections.nextAction} onToggle={() => toggleSection('nextAction')} incomplete={!lead.nextAction && lead.stage !== 'CONVERTED'}>
        <AbNextAction label={t('lead.fields.nextAction')} title={lead.nextAction ?? '-'} due={formatDate(lead.nextActionAt, i18n.language, true)} action={lead.stage !== 'CONVERTED' ? <button type="button" className="lead-v2-button ghost" onClick={() => openActivityTab()}>{t('lead.tabs.activity')}</button> : undefined} />
      </AbSectionAccordion>
    ...
    const conversionResult = conversionResultFor(lead);
    return <AbSectionAccordion id="conversion" title={t('lead.sections.conversion')} open={openSections.conversion} onToggle={() => toggleSection('conversion')}>
      <LeadConversionPanel
        entityKey={lead.leadId}
        canConvert={lead.stage !== 'DISQUALIFIED' && lead.stage !== 'CONVERTED'}
        converted={lead.stage === 'CONVERTED'}
        result={conversionResult}
        accounts={accountOptions}
        defaultOpportunityName={t('lead.conversion.defaultOpportunityName', { account: lead.organizationName })}
        onConvert={options => convertLead(lead, options)}
        onExclude={() => updateStage('DISQUALIFIED')}
        excludeDisabled={lead.stage === 'CONVERTED'}
      />
    </AbSectionAccordion>;
  };
  ...
  const detailPane = <>{!selected && <div className="lead-v2-empty-detail">{t('lead.empty.detail')}</div>}{selected && <>
    ...
    <AbDetailHeader
      ...
      badges={<AbEntityBadges badges={[
        { label: t(`leadV2.stage.${selected.stage}`), tone: selected.stage === 'CONVERTED' ? 'success' : selected.stage === 'DISQUALIFIED' ? 'warning' : 'info' },
    ...
    />
    <AbStepProgress steps={stepLabels} currentIndex={Math.max(0, mockStageToStep(selected.stage))} mobileLabel={selected.stage === 'DISQUALIFIED' ? t('lead.status.CONTACT_EXCLUDED') : `${Math.max(0, mockStageToStep(selected.stage)) + 1}/${API_LEAD_STEPS.length}`} />
    ...
    <div className="ab-detail-control-strip">
      <label><span>{t('lead.fields.stage')}</span><select value={selected.stage} disabled={selected.stage === 'CONVERTED'} onChange={event => updateStage(event.target.value as LeadStage)}>{LEAD_STAGES.map(stage => <option key={stage} value={stage}>{t(`leadV2.stage.${stage}`)}</option>)}</select></label>
    ...
    <AbDetailTabs ariaLabel={t('lead.detailTitle')} activeId={activeTab} onChange={selectDetailTab} tabs={[
      ...
      { id: 'conversion', label: t('lead.tabs.conversion'), count: selected.convertedAccountId ? 1 : selected.opportunityCount }
    ]} />
  ...
  return <section className={`lead-v2 ab-workspace${mobileDetailOpen ? ' mobile-detail-open' : ''}`}>
    <header className="lead-v2-page-header">
      <div><div className="lead-v2-title-line"><span className="lead-v2-kicker">CRM · LEAD</span><span className="lead-v2-mock-badge">{t('lead.mockMode')}</span></div><h2>{t('lead.title')}</h2><p>{t('lead.subtitle')}</p></div>
      <div className="lead-v2-header-actions"><button type="button" className="lead-v2-button ghost" onClick={resetMock}>{t('lead.actions.resetMock')}</button><button type="button" className="lead-v2-button primary" onClick={() => setDrawerOpen(true)}>+ {t('lead.actions.create')}</button></div>
    </header>
    ...
    <LeadQuickCreatePanel
      open={drawerOpen}
      defaultCountry="KR"
      ownerOptions={MOCK_LEAD_OWNERS.map(owner => ({ value: owner.id, label: owner.name }))}
