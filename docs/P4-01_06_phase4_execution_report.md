...
`APPROVED / CI_PASS / DB_MIGRATION_DEFERRED`

Phase 4 P4-01~P4-06 Source Baseline 구현과 GitHub Actions Build/Test 검증을 완료했고, 2026-09-12 사용자 지시로 Human Domain Review를 승인한다. 운영/DEV DB Migration은 별도 환경 Gate로 계속 연기한다.

## P4-01 Opportunity CRUD
- Lead Convert 생성 Opportunity와 기존 Account 신규 Opportunity를 동일 Domain에서 관리
- 목록/상세/생성/수정
- Delete는 물리삭제가 아닌 Logical Archive로 처리하고 History 유지
...
- 예상마감일, 성공확률, Forecast Category
- 관심품목, 특약조건
- 결제방법/결제일/할부개월
- 경쟁사 사용현황, 보유장비, 진료비 관련정보

## P4-05 Closed Won Validation
- Closed Won 변경 시 Account의 `erp_approved_yn=1` 필수
- ERP 미승인 Account는 Closed Won 차단
- Opportunity Approval 프로세스는 GAP-009가 미확정이므로 임의 구현하지 않음
- Contract 생성/수금계획은 Phase 5 범위

## P4-06 Pipeline / Funnel
- Stage별 Opportunity Count / Amount
- 성공확률 기반 Weighted Amount
- Forecast Category별 Count/Amount
- 담당자/예상마감일 기간 필터 API

## CI Result
최종 GitHub Actions Run `34674291024` PASS.
- install: PASS
- shared contracts: PASS
- NestJS backend: PASS
- React/Vite frontend: PASS
- Jest / Opportunity Domain Rule: PASS

초기 CI에서 Phase 3 `ActivityCalendarItem` 공유타입 회귀를 검출했고 기존 Phase 3 contract를 보존하도록 수정했다.
