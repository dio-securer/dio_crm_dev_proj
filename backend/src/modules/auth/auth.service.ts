...
type LoginRow = {
  user_id: number; public_id: string; login_id: string; user_name: string;
  company_id: number; organization_id: number | null; password_hash: string;
};
...
export class AuthService {
  ...
  async login(loginId: string, password: string) {
    const userResult = await this.db.query<LoginRow>(`
    ...
    const payload: JwtPayload = {
      sub: row.user_id, publicId: row.public_id, loginId: row.login_id, name: row.user_name,
      companyId: row.company_id, organizationId: row.organization_id ?? undefined,
    ...
    const refreshToken = await this.jwt.signAsync({ sub: row.user_id, type: 'refresh' }, {
    ...
    `, { userId: row.user_id, hash, days: env.JWT_REFRESH_EXPIRES_DAYS });
  ...
  async refresh(refreshToken: string) {
    ...
    const stored = await this.db.query<{ refresh_token_id: number }>(`
      SELECT TOP 1 refresh_token_id FROM crm_refresh_token
      WHERE user_id=@userId AND token_hash=@hash AND revoked_at IS NULL AND expires_at>SYSUTCDATETIME()
    `, { userId: token.sub, hash });
    if (!stored.recordset[0]) throw new UnauthorizedException('Refresh token revoked or expired');
    return { accepted: true, userId: token.sub };
  }

  async logout(refreshToken: string) {
    const hash = crypto.createHash('sha256').update(refreshToken).digest('hex');
    await this.db.query(`UPDATE crm_refresh_token SET revoked_at=SYSUTCDATETIME() WHERE token_hash=@hash`, { hash });
    return { success: true };
  }
}
